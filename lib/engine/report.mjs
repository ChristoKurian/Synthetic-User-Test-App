...
export function renderReportHtml(title, stats, opts = {}) {
  // Without a real success signal, no session can ever register as
  // "success" — engine.mjs's checkCriteria always returns null against an
  // empty criteria list. A bare "0%" in that case isn't a finding about
  // the prototypes, it's a guaranteed artifact of not having a signal to
  // check. Rather than show a fake zero, say plainly that success wasn't
  // measured and lead with the metrics that ARE real in that mode
  // (time/clicks/friction), which is exactly what the UI already promises
  // when this field is left blank.
  const hasSuccessSignal = opts.hasSuccessSignal !== false;
  ...
  if (hasSuccessSignal) {
    ...
    callout = `<strong>${esc(best)}</strong> had the highest task success rate (${pct(bestStats.successRate)} of ${bestStats.n} synthetic sessions), median time-to-success ${fmtMs(bestStats.medianTimeToSuccessMs)}.`;
  } else {
    const fewestClicks = names.reduce((a, b) => (stats[a].medianClicks <= stats[b].medianClicks ? a : b), names[0]);
    callout = `No success signal was set, so success rate isn't measured here — every session ran until it either finished exploring or hit its time limit, which is expected, not a failure. Comparing by friction instead: <strong>${esc(fewestClicks)}</strong> had the fewest median clicks (${stats[fewestClicks].medianClicks.toFixed(1)}). Check the error/rage-click/backtrack columns below too, and consider adding a success signal (e.g. a confirmation message) for a real completion-rate comparison.`;
  }
